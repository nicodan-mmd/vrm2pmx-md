    ...
    def load(self, file_idx=0, is_check=True):
        ...
        try:
            ...
            if new_data_digest and (
                (self.data and current_digest != new_data_digest) or not self.data
            ):
    ...
    def OnDropFiles(self, x, y, filenames):
        ...
        if os.path.isdir(filenames[0]) and self.is_aster:
            ...
            child_file_name_exts = [
                ...
                if os.path.isfile(os.path.join(filenames[0], filename))
            ]

            for ft in self.parent.file_type:
                # 親の許容ファイルパス
                for _, child_file_ext in child_file_name_exts:
        ...
        if isinstance(self.parent.file_type, tuple):
