...
class TargetBoneDialog(wx.Dialog):
    ...
    def __init__(self, frame: Any, panel: Any, direction: str, type: str):
        super().__init__(
            ...
            size=wx.Size(850, 450),
            style=wx.DEFAULT_DIALOG_STYLE,
            name="TargetBoneDialog",
        )
        ...
        self.vmd_digest = (
            0
            if not self.panel.vmd_file_ctrl.data
            else self.panel.vmd_file_ctrl.data.digest
        )
        self.pmx_digest = (
            0
            if not self.panel.model_file_ctrl.data
            else self.panel.model_file_ctrl.data.digest
        )
        ...
        self.description_txt = wx.StaticText(
            self,
            wx.ID_ANY,
            f"多段{type}したいボーン名を選択・入力してください。プルダウン欄にボーン名の一部を入力して絞り込みをかける事ができます。\n"
            + "プルダウン欄にボーン名を入力した場合、変換ENTERの後、もう一度ENTERを押すと、移動・回転の各ボーンに同ボーン名が入ります。\n"
            + "多段ボーンは3つまで指定する事ができます。軸ごとに中身の成分が分かれていてもごちゃ混ぜでも、どちらでもOKです。",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
        ...
        self.import_btn_ctrl = wx.Button(
            self, wx.ID_ANY, "インポート ...", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.import_btn_ctrl.SetToolTip(
            "ボーンデータをCSVファイルから読み込みます。\nファイル選択ダイアログが開きます。"
        )
        ...
        self.export_btn_ctrl = wx.Button(
            self, wx.ID_ANY, "エクスポート ...", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.export_btn_ctrl.SetToolTip(
            "ボーンデータをCSVファイルに出力します。\n調整対象VMDと同じフォルダに出力します。"
        )
        ...
        self.add_line_btn_ctrl = wx.Button(
            self, wx.ID_ANY, "行追加", wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        self.org_model_name_txt = wx.StaticText(
            self.window, wx.ID_ANY, "モデルなし", wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        self.rotate_x_txt = wx.StaticText(
            self.window, wx.ID_ANY, "回転(X)", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.rotate_x_txt.Wrap(-1)
        self.grid_sizer.Add(self.rotate_x_txt, 0, wx.ALL, 5)

        self.rotate_y_txt = wx.StaticText(
            self.window, wx.ID_ANY, "回転(Y)", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.rotate_y_txt.Wrap(-1)
        self.grid_sizer.Add(self.rotate_y_txt, 0, wx.ALL, 5)

        self.rotate_z_txt = wx.StaticText(
            self.window, wx.ID_ANY, "回転(Z)", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.rotate_z_txt.Wrap(-1)
        self.grid_sizer.Add(self.rotate_z_txt, 0, wx.ALL, 5)

        self.move_x_txt = wx.StaticText(
            self.window, wx.ID_ANY, "移動(X)", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.move_x_txt.Wrap(-1)
        self.grid_sizer.Add(self.move_x_txt, 0, wx.ALL, 5)

        self.move_y_txt = wx.StaticText(
            self.window, wx.ID_ANY, "移動(Y)", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.move_y_txt.Wrap(-1)
        self.grid_sizer.Add(self.move_y_txt, 0, wx.ALL, 5)

        self.move_z_txt = wx.StaticText(
            self.window, wx.ID_ANY, "移動(Z)", wx.DefaultPosition, wx.DefaultSize, 0
        )
    ...
    def initialize(self):
        if self.panel.model_file_ctrl.data:
            self.org_model_name_txt.SetLabel(
                self.panel.vmd_file_ctrl.data.model_name[:10]
            )
        ...
        with wx.FileDialog(
            ...
            try:
                with open(target_bone_path, "r") as f:
                    ...
                    for ov, rmxv, rmyv, rmzv, rrxv, rryv, rrzv in zip(
                        org_choice_values,
                        rep_mx_choice_values,
                        rep_my_choice_values,
                        rep_mz_choice_values,
    ...
    def get_bone_list(self):
        ...
        for oc, rmxc, rmyc, rmzc, rrxc, rryc, rrzc in zip(
            self.org_choices,
            self.rep_mx_choices,
            self.rep_my_choices,
            self.rep_mz_choices,
            ...
            if oc.GetSelection() > 0 and (
                rmxc.GetSelection() > 0
                or rmyc.GetSelection() > 0
                or rmzc.GetSelection() > 0
                or rrxc.GetSelection() > 0
                or rryc.GetSelection() > 0
                or rrzc.GetSelection() > 0
            ):
    ...
    def add_line(self, midx=0):
        ...
        self.org_choices.append(
            wx.ComboBox(
                self.window,
                id=wx.ID_ANY,
                choices=self.org_bones,
                style=wx.CB_DROPDOWN | wx.TE_PROCESS_ENTER,
            )
        )
        self.org_choices[-1].Bind(
            wx.EVT_COMBOBOX, lambda event: self.on_change_choice(event, midx)
        )
        self.org_choices[-1].Bind(
            wx.EVT_TEXT_ENTER, lambda event: self.on_enter_choice(event, midx)
        )
        ...
        self.rep_ry_choices[-1].Bind(
            wx.EVT_TEXT_ENTER, lambda event: self.on_enter_choice(event, midx)
        )
        self.grid_sizer.Add(self.rep_ry_choices[-1], 0, wx.ALL, 5)

        # 置換後ボーン(RZ)
        self.rep_rz_choices.append(
            wx.ComboBox(
                self.window,
                id=wx.ID_ANY,
                choices=self.rep_bones,
        ...
        self.rep_rz_choices[-1].Bind(
            wx.EVT_TEXT_ENTER, lambda event: self.on_enter_choice(event, midx)
        )
        self.grid_sizer.Add(self.rep_rz_choices[-1], 0, wx.ALL, 5)

        # 置換後ボーン(MX)
        self.rep_mx_choices.append(
            wx.ComboBox(
                self.window,
                id=wx.ID_ANY,
                choices=self.rep_bones,
        ...
        self.rep_mx_choices[-1].Bind(
            wx.EVT_TEXT_ENTER, lambda event: self.on_enter_choice(event, midx)
        )
        self.grid_sizer.Add(self.rep_mx_choices[-1], 0, wx.ALL, 5)

        # 置換後ボーン(MY)
        self.rep_my_choices.append(
            wx.ComboBox(
                self.window,
                id=wx.ID_ANY,
                choices=self.rep_bones,
        ...
        self.rep_my_choices[-1].Bind(
            wx.EVT_TEXT_ENTER, lambda event: self.on_enter_choice(event, midx)
        )
        self.grid_sizer.Add(self.rep_my_choices[-1], 0, wx.ALL, 5)

        # 置換後ボーン(MZ)
        self.rep_mz_choices.append(
            wx.ComboBox(
                self.window,
                id=wx.ID_ANY,
                choices=self.rep_bones,
    ...
    def on_change_choice(self, event: wx.Event, midx: int):
        ...
        if (
            ...
            and (
                len(self.rep_mx_choices[midx].GetStringSelection()) > 0
                or len(self.rep_my_choices[midx].GetStringSelection()) > 0
                or len(self.rep_mz_choices[midx].GetStringSelection()) > 0
                or len(self.rep_rx_choices[midx].GetStringSelection()) > 0
                or len(self.rep_ry_choices[midx].GetStringSelection()) > 0
                or len(self.rep_rz_choices[midx].GetStringSelection()) > 0
            )
        ):
