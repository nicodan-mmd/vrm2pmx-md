...
class RigidbodyPanel(BasePanel):
    ...
    def __init__(self, frame: Any, export: wx.Notebook, tab_idx: int):
        ...
        self.rigidbody_set_list_sizer.Add(
            self.rigidbody_set.rigidbody_set_sizer, 0, wx.EXPAND | wx.ALL, 5
        )
    ...
    def get_bone_pairs(self):
        ...
        for (
            vrm_bone_txt,
            pmx_rigidbody_factor_txt,
            pmx_rigidbody_param_mass_txt,
            pmx_rigidbody_param_linear_damping_txt,
            pmx_rigidbody_param_angular_damping_txt,
            pmx_rigidbody_param_restitution_txt,
            pmx_rigidbody_param_friction_txt,
        ) in zip(
            self.rigidbody_set.vrm_bone_txts,
            self.rigidbody_set.pmx_rigidbody_factor_txts,
            self.rigidbody_set.pmx_rigidbody_param_mass_txts,
            self.rigidbody_set.pmx_rigidbody_param_linear_damping_txts,
            self.rigidbody_set.pmx_rigidbody_param_angular_damping_txts,
            self.rigidbody_set.pmx_rigidbody_param_restitution_txts,
            self.rigidbody_set.pmx_rigidbody_param_friction_txts,
            strict=False,
        ):
            bone_pairs[vrm_bone_txt][
                "rigidbodyFactor"
            ] = pmx_rigidbody_factor_txt.GetValue()
            bone_pairs[vrm_bone_txt]["rigidbodyParam"][
                0
            ] = pmx_rigidbody_param_mass_txt.GetValue()
            bone_pairs[vrm_bone_txt]["rigidbodyParam"][
                1
            ] = pmx_rigidbody_param_linear_damping_txt.GetValue()
            bone_pairs[vrm_bone_txt]["rigidbodyParam"][
                2
            ] = pmx_rigidbody_param_angular_damping_txt.GetValue()
            bone_pairs[vrm_bone_txt]["rigidbodyParam"][
                3
            ] = pmx_rigidbody_param_restitution_txt.GetValue()
            bone_pairs[vrm_bone_txt]["rigidbodyParam"][
                4
            ] = pmx_rigidbody_param_friction_txt.GetValue()
        return bone_pairs
                        ...
                        if row and row[0] in self.rigidbody_set.vrm_bone_txts:
                            rigidbody_idx = [
                                ...
                                for i, r in enumerate(
                                    list(self.rigidbody_set.vrm_bone_txts)
                                )
                                if r == row[0]
                            ][0]
                            self.rigidbody_set.pmx_rigidbody_factor_txts[
                                rigidbody_idx
                            ].SetValue(float(row[2]))
                            self.rigidbody_set.pmx_rigidbody_param_mass_txts[
                                rigidbody_idx
                            ].SetValue(float(row[3]))
                            self.rigidbody_set.pmx_rigidbody_param_linear_damping_txts[
                                rigidbody_idx
                            ].SetValue(float(row[4]))
                            self.rigidbody_set.pmx_rigidbody_param_angular_damping_txts[
                                rigidbody_idx
                            ].SetValue(float(row[5]))
                            self.rigidbody_set.pmx_rigidbody_param_restitution_txts[
                                rigidbody_idx
                            ].SetValue(float(row[6]))
                            self.rigidbody_set.pmx_rigidbody_param_friction_txts[
                                rigidbody_idx
                            ].SetValue(float(row[7]))
                        ...
                        if bone_dict["rigidBody"]:
                            cw.writerow(
                                [
                                    vrm_bone_name,
                                    bone_dict["name"],
                                    bone_dict["rigidbodyFactor"],
                                    bone_dict["rigidbodyParam"][0],
                                    bone_dict["rigidbodyParam"][1],
                                    bone_dict["rigidbodyParam"][2],
                                    bone_dict["rigidbodyParam"][3],
                                    bone_dict["rigidbodyParam"][4],
                                ]
                            )
...
class RigidbodySet:
    ...
    def __init__(self, frame: Any, panel: wx.Panel, window: wx.Window):
        ...
        for text, tooltip, width in [
            ("PMXボーン名", "VRMモデルから変換したPMXボーン名（準標準まで）", 80),
            ("大きさ係数", "PMXモデルの剛体の大きさ", 65),
            ("質量", "PMXモデルの剛体の質量", 65),
            ("移動減衰", "根元剛体の移動減衰（末端との線形補間）", 65),
            ("回転減衰", "根元剛体の回転減衰（末端との線形補間）", 65),
            ("反発力", "根元剛体の反発力（末端との線形補間）", 65),
            ("摩擦力", "根元剛体の摩擦力（末端との線形補間）", 65),
        ]:
            txt_ctrl = wx.StaticText(
                self.window, wx.ID_ANY, text, wx.DefaultPosition, wx.Size(width, -1), 0
            )
            if tooltip:
        ...
        for vrm_bone_name, bone_pair in BONE_PAIRS.items():
            if bone_pair["rigidBody"]:
                ...
                self.pmx_bone_txts.append(
                    wx.TextCtrl(
                        self.window,
                        id=wx.ID_ANY,
                        value=bone_pair["name"],
                ...
                self.pmx_rigidbody_factor_txts.append(
                    wx.SpinCtrlDouble(
                        self.window,
                        id=wx.ID_ANY,
                        size=wx.Size(65, -1),
                        value=f"{bone_pair['rigidbodyFactor']}",
                        min=0,
                        max=999,
                        initial=bone_pair["rigidbodyFactor"],
                        inc=0.001,
                    )
                )
                ...
                self.pmx_rigidbody_param_mass_txts.append(
                    wx.SpinCtrlDouble(
                        ...
                        id=wx.ID_ANY,
                        size=wx.Size(65, -1),
                        value=f"{bone_pair['rigidbodyParam'][0]}",
                        min=0,
                        max=999,
                        initial=bone_pair["rigidbodyParam"][0],
                        inc=0.001,
                    )
                )
                self.pmx_rigidbody_param_mass_txts[-1].Bind(
                    wx.EVT_MOUSEWHEEL,
                    lambda event: self.frame.on_wheel_spin_ctrl(event, 0.001),
                )
                self.grid_sizer.Add(
                    self.pmx_rigidbody_param_mass_txts[-1], 0, wx.ALL, 5
                )
                ...
                self.pmx_rigidbody_param_linear_damping_txts.append(
                    wx.SpinCtrlDouble(
                        ...
                        id=wx.ID_ANY,
                        size=wx.Size(65, -1),
                        value=f"{bone_pair['rigidbodyParam'][1]}",
                        min=0,
                        max=999,
                        initial=bone_pair["rigidbodyParam"][1],
                        inc=0.001,
                    )
                )
                self.pmx_rigidbody_param_linear_damping_txts[-1].Bind(
                    wx.EVT_MOUSEWHEEL,
                    lambda event: self.frame.on_wheel_spin_ctrl(event, 0.001),
                )
                self.grid_sizer.Add(
                    self.pmx_rigidbody_param_linear_damping_txts[-1], 0, wx.ALL, 5
                )
                ...
                self.pmx_rigidbody_param_angular_damping_txts.append(
                    wx.SpinCtrlDouble(
                        ...
                        id=wx.ID_ANY,
                        size=wx.Size(65, -1),
                        value=f"{bone_pair['rigidbodyParam'][2]}",
                        min=0,
                        max=999,
                        initial=bone_pair["rigidbodyParam"][2],
                        inc=0.001,
                    )
                )
                self.pmx_rigidbody_param_angular_damping_txts[-1].Bind(
                    wx.EVT_MOUSEWHEEL,
                    lambda event: self.frame.on_wheel_spin_ctrl(event, 0.001),
                )
                self.grid_sizer.Add(
                    self.pmx_rigidbody_param_angular_damping_txts[-1], 0, wx.ALL, 5
                )
                ...
                self.pmx_rigidbody_param_restitution_txts.append(
                    wx.SpinCtrlDouble(
                        ...
                        id=wx.ID_ANY,
                        size=wx.Size(65, -1),
                        value=f"{bone_pair['rigidbodyParam'][3]}",
                        min=0,
                        max=999,
                        initial=bone_pair["rigidbodyParam"][3],
                        inc=0.001,
                    )
                )
                self.pmx_rigidbody_param_restitution_txts[-1].Bind(
                    wx.EVT_MOUSEWHEEL,
                    lambda event: self.frame.on_wheel_spin_ctrl(event, 0.001),
                )
                self.grid_sizer.Add(
                    self.pmx_rigidbody_param_restitution_txts[-1], 0, wx.ALL, 5
                )
                ...
                self.pmx_rigidbody_param_friction_txts.append(
                    wx.SpinCtrlDouble(
                        ...
                        id=wx.ID_ANY,
                        size=wx.Size(65, -1),
                        value=f"{bone_pair['rigidbodyParam'][4]}",
                        min=0,
                        max=999,
                        initial=bone_pair["rigidbodyParam"][4],
                        inc=0.001,
                    )
                )
                self.pmx_rigidbody_param_friction_txts[-1].Bind(
                    wx.EVT_MOUSEWHEEL,
                    lambda event: self.frame.on_wheel_spin_ctrl(event, 0.001),
                )
                self.grid_sizer.Add(
                    self.pmx_rigidbody_param_friction_txts[-1], 0, wx.ALL, 5
                )
