...
class BonePanel(BasePanel):
    ...
    def __init__(self, frame: wx.Frame, export: wx.Notebook, tab_idx: int):
        ...
        self.description_txt = wx.StaticText(
            self.header_panel,
            wx.ID_ANY,
            "Vroid Studio のボーン命名をMMD用に設定します。",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
    ...
    def get_bone_pairs(self):
        bone_pairs = {}
        for vrm_bone_txt, pmx_bone_txt in zip(
            self.bone_set.vrm_bone_txts, self.bone_set.pmx_bone_txts, strict=False
        ):
...
class BoneSet:
    ...
    def __init__(self, frame: Any, panel: wx.Panel, window: wx.Window):
        ...
        self.vrm_bone_header_txt = wx.StaticText(
            self.window,
            wx.ID_ANY,
            "VRMボーン名",
            wx.DefaultPosition,
        ...
        self.pmx_bone_header_txt = wx.StaticText(
            self.window,
            wx.ID_ANY,
            "PMXボーン名",
            wx.DefaultPosition,
        ...
        for bone_pair in BONE_PAIRS:
            ...
            self.pmx_bone_txts.append(
                wx.TextCtrl(
                    self.window, id=wx.ID_ANY, value=bone_pair[1], size=wx.Size(180, -1)
                )
            )
