...
from utils import MFileUtils, MFormUtils
...
class ExportPanel(BasePanel):
    ...
    def __init__(self, frame: Any, export: wx.Notebook, tab_idx: int):
        ...
        self.description_txt = wx.StaticText(
            self,
            wx.ID_ANY,
            "Vroid Studio で作成された VRMデータをMMD用の準標準ボーンっぽいPMXデータに変換します。\n"
            + "※Vroid Studio 以外のツール（Blender等）を介して生成されたVrmモデルは、\n"
            + "データ形式が想定外なので変換に失敗します。",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
        ...
        # 対象Vrmファイルコントロール
        self.vrm_model_file_ctrl = HistoryFilePickerCtrl(
            self.frame,
            self,
            "対象モデルVRM",
            "対象モデルVRMファイルを開く",
            ...
            "変換したいVRMファイルパスを指定してください。\nD&Dでの指定、開くボタンからの指定、履歴からの選択ができます。",
            file_model_spacer=0,
            title_parts_ctrl=None,
            title_parts2_ctrl=None,
            file_histories_key="export_vrm",
        ...
        # 出力先Pmxファイルコントロール
        self.output_pmx_file_ctrl = BaseFilePickerCtrl(
            frame,
            self,
            "出力対象PMX",
            "出力対象PMXファイルを開く",
        ...
        self.export_btn_ctrl = wx.Button(
            self, wx.ID_ANY, "VRM2PMX変換", wx.DefaultPosition, wx.Size(200, 50), 0
        )
        self.export_btn_ctrl.SetToolTip("VRMモデルをMMD用PMXモデルに変換します")
    ...
    def set_output_vmd_path(self, event, is_force=False):
        output_pmx_path = MFileUtils.get_output_pmx_path(
            ...
            self.output_pmx_file_ctrl.file_ctrl.GetPath(),
            is_force,
        )
    ...
    def on_convert(self, event: wx.Event):
        ...
        # VRM2PMX変換開始
        if (
            self.export_btn_ctrl.GetLabel() == "VRM2PMX停止"
            and self.convert_export_worker
        ):
            # フォーム無効化
            self.disable()
            # 停止状態でボタン押下時、停止
